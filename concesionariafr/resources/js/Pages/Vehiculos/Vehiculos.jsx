...
const Vehiculos  = ({
    ...
    const [favoriteVehiculos, setFavoriteVehiculos] = useState([]);
    ...
    useEffect(() => {
        if (user) {
            // Obtener los vehículos favoritos del usuario al cargar el componente
            axios
                .get(route("favoritos.list", { clienteId: user.id }))
                .then((response) => {
                    setFavoriteVehiculos(response.data);
                })
    ...
    const handleAddToFavorites = (vehiculoId) => {
        axios
            ...
            .then(() => {
                setFavoriteVehiculos((prev) => [...prev, vehiculoId]);
            })
